...
def start(bot: Bot, update: Update, args: List[str]):
    ...
    if update.effective_chat.type == "private":
        if args:
            if args[0].lower() == "help":
                ...
                if is_user_admin(chat, update.effective_user.id):
                    send_settings(match[1], update.effective_user.id, user=False)
                else:
                    send_settings(match[1], update.effective_user.id, user=True)
        ...
        update.effective_message.reply_text(
            f"Hello [{msg.from_user.first_name}](tg://user?id={msg.from_user.id}), Wassup",
            parse_mode=ParseMode.MARKDOWN,
        )
    ...
    keyboard += [
        [
            InlineKeyboardButton(
                text=tld(chat.id, "Add me to group"),
